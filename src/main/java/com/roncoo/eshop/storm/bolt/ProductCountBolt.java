                ...
                try {
                    ...
                    for (Map.Entry<Long, Long> productCountEntry : productCountList) {
                        if (productCountEntry.getValue() > 10 * avgCount) {
                            ...
                            String distributeNginxURL = "http://192.168.8.83/hot?productId=" + productCountEntry.getKey();
                            HttpClientUtils.sendGetRequest(distributeNginxURL);

                            // 将缓存热点，那个商品对应的完整的缓存数据，发送请求到缓存服务去获取，反向推送到所有的后端应用nginx服务器上去
                            String cacheServiceURL = "http://192.168.8.81:8080/getProductInfo?productId=" + productCountEntry.getKey();
                            String response = HttpClientUtils.sendGetRequest(cacheServiceURL);

                            String[] appNginxURLs = new String[]{
                                    "http://192.168.8.81/hot?productId=" + productCountEntry.getKey() + "&productInfo=" + response,
                                    "http://192.168.8.82/hot?productId=" + productCountEntry.getKey() + "&productInfo=" + response
                            };
                    ...
                    if (lastTimeHotProductIdList.size() == 0) {
                        ...
                        for (Long productId : lastTimeHotProductIdList) {
                            ...
                            if (!hotProductIdList.contains(productId)) {
                                //说明上一次那个商品id的热点消失了
                                //发送一个http请求给到流量分发的nginx中，取消热点缓存的 标识
                                String url = "http://192.168.8.83/cancel_hot?productId=" + productId;
